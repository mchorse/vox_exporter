...
import mchorse.vox.Vox.VoxChunk;
...
public class VoxReader
{
    ...
    public Vox read(InputStream stream) throws Exception
    {
        if (this.readInt(stream) != this.fourChars('V', 'O', 'X', ' '))
        {
        ...
        if (main.id != this.fourChars('M', 'A', 'I', 'N'))
        {
        ...
        boolean readXYZ = false;

        while (true)
        {
            VoxChunk chunk = this.readChunk(stream);
            ...
            else if (chunk.id == this.fourChars('S', 'I', 'Z', 'E'))
            {
                if (readSize)
                {
            ...
            else if (chunk.id == this.fourChars('X', 'Y', 'Z', 'I'))
            {
                if (readXYZ)
                {
                    stream.skip(chunk.size);
            ...
            else if (chunk.id == this.fourChars('R', 'G', 'B', 'A'))
            {
                vox.palette = new int[256];
            ...
            if (vox.palette != null && vox.voxels != null)
            {
                break;
            }
        }
        ...
        if (vox.palette == null)
        {
            vox.palette = Arrays.copyOf(DEFAULT_PALETTE, DEFAULT_PALETTE.length);
        }
    ...
    {
        VoxChunk chunk = new Vox.VoxChunk();

        chunk.id = this.readInt(stream);
        chunk.size = this.readInt(stream);
        chunk.chunks = this.readInt(stream);
